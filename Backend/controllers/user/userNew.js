...
const userNew = async (req, res, next) => {
    ...
    try {
        ...
        if (!name || !username || !email || !password) {
            throw generateError(
                '¿Seguro que has puesto todos los datos necesarios? Hacen falta campos obligatrios',
                400
            );
        }
        ...
        await connection.query(`select id from user where username = ?`, [
            username,
        ]);
        if (user.length > 0) {
            throw generateError(
   ...
   insert into user (name, username, email, password, createdAt) values (?,?,?,?,?,?)`,
            [name, username, email, password, new Date()]
        );
